...
#import "SACalendarExpire.h"
#import "SACalendarCell.h"
#import "DMLazyScrollView.h"
#import "DateUtil.h"

@interface SACalendarExpire () <UICollectionViewDataSource, UICollectionViewDelegate>{
...
@implementation SACalendarExpire
...
- (void) observeValueForKeyPath:(NSString*)keyPath ofObject:(id)object change:(NSDictionary*)change context:(void*)context {
    if (nil != _delegate && [_delegate respondsToSelector:@selector(SACalendarExpire:didDisplayCalendarForMonth:year:)]) {
        [_delegate SACalendarExpire:self didDisplayCalendarForMonth:month year:year];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!(indexPath.row < firstDay || indexPath.row >= firstDay + daysInMonth)) {
        ...
        if (nil != _delegate && [_delegate respondsToSelector:@selector(SACalendarExpire:didSelectDate:month:year:)]) {
            [_delegate SACalendarExpire:self didSelectDate:dateSelected month:month year:year];
        }
        
        //Test
        
        if (nil != _delegate && [_delegate respondsToSelector:@selector(SACalendarExpire:didSelectDateExpire:month:year:)]) {
            [_delegate SACalendar:self didSelectDateExpire:dateSelected month:month year:year];
        }
